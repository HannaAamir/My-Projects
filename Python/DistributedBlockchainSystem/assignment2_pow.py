...
def solve_proof_of_work(previous_hash, user_input):
    ...
    while True:
        ...
        input_data = str(nonce) + previous_hash + user_input
...
def main():
    ...
    while True:
        ...
        if choice == "3":
            ...
            # Find the valid nonce by calling the function
            nonce, current_hash = solve_proof_of_work(previous_hash, user_input)
            # Compute current block hash
            # generate a new block
            new_block = Block(block_index, nonce, user_input, previous_hash, current_hash)
            # Append the block to the blockchain
            blockchain.append(new_block)
            print(f"New block created with nonce: {nonce}") # complete this to print nonce
            print(f"Hash of the new block: {current_hash}") # complete this to prin thash of new block.
