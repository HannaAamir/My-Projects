...
double computeArea(ShapeType shape, double length1, double length2 = 0) { //remove default arguments from definition + put only in declaration + must be only in the rightmost position of all paraaters in function prototype and function definition
    ...
    switch (shape) {
        ...
        case ShapeType::TRIANGLE:
            area = (length1 * length2)/2.0; //ASSUMING LENGTH1 IS BASE AND LENGTH2 IS HEIGHT
            break;
...
double computePerimeter(ShapeType  shape, double length1, double length2 = 0) {
    ...
    switch (shape) {
        ...
        case ShapeType::SQUARE:
            ...
            break;
        case ShapeType::TRIANGLE:
            perimeter = (length1*2) + length2; //assuming length1 is side or based, NEEDS TO BE FIXED
            break;
        case ShapeType::CIRCLE:
...
void moveRight (int numOfHorizontalSpaces = default_horizontal_spaces) { //make it equivalent so that in moveup, move right matches the previus function call
...
void draw(ShapeType  shape, double length1, double length2, char lineType = '*') {
    ...
    double square_height_adjustment = length1 - 2.0; //len2 is the same as len1 for a square
    ...
    switch (shape) {
        case ShapeType::RECTANGLE:
            std::cout << string(length1, lineType);
            moveUp();
            
            for (double h = 0.0; h < rectangle_height_adjustment; h++){
                std::cout << lineType;
                cout << string(length1 - 2.0, space); //maybe make seperate variable instead of hardcoding
                std::cout << lineType << endl;
                moveUp();
            }
        ...
        case ShapeType::SQUARE:
            ...
            for (double h = 0.0; h < (square_height_adjustment)/2.0; h++){
                cout << lineType;
        ...
        case ShapeType::TRIANGLE:
            ...
            cout << string(length1, lineType) << endl;
...
int main(int argc, const char * argv[]) {
    // insert code here...
    moveRight(24);
    draw(CIRCLE, 2, 0); //0 should be removed 
    moveRight(20);
    draw(TRIANGLE, 10, 5);
    moveRight(20);
    draw(SQUARE, 10, 0);
